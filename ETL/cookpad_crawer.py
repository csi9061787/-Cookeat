...
# 建立MongoDB數據庫連接 IP='***.***.***.***', port=27017
# uri = 'mongodb://root:root@***.***.***.***:27017/?authSource=admin&readPreference=primary&appname=MongoDB%20Compass&ssl=false'
# client = MongoClient(uri)
# db = client['Recipe_OriginData']
# collection = db['cookpad']

# ----測試連本機內虛擬機docker環境的MongoDB----
uri = 'mongodb://root:root@***.***.***.***:27017/?authSource=admin&readPreference=primary&appname=MongoDB%20Compass&ssl=false'
client = MongoClient(uri)
db = client['test']  # 測試用
    ...
    while True:
        try:
            ...
            for Recipe in TotalRecipes:
                ...
                # print('類別: ', SubCategoryName)
                # print('類別網址: ', SubCategoryURL)
                # print('食譜網址: ', RecipeURL)
                try:
                    # ----食譜名稱及敘述----
                    RecipeName = RecipeSoup.select('section[class="intro-container document-section"] > h1')[0].text.replace('\n', '').strip().replace('/', '_')
                    Description = RecipeSoup.select('div[data-collapse-target="content"]')[0].text.replace('\n', '').strip()
                    # print('食譜名稱: ', RecipeName)
                    # print('食譜說明: ', Description)
                    ...
                    # print('作者ID: ', AuthorID)
                    # print('作者名稱: ', AuthorName)

                    # ----料理時間及份數----
                    if len(RecipeSoup.select('div > span > span[class="mise-icon-text"]')) == 0:  # 判斷是否有給料理時間
                        CookTime = None
                    else:
